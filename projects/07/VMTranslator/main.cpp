...
int main(int argc, char **argv)
{
	...
	cout << "asm_path: " << asm_path.string() << endl
		 << endl;
	...
	codeWriter.close();
	if (debug)
		cout << "VM Translation Ended!" << endl;
	return 0;
}
...
void translate(Parser &parser, CodeWriter &codeWriter, fs::path vmfile)
{
	...
	while (parser.hasMoreCommands())
	{
		...
		{
			if (debug)
				cout << "[" << lineNumber << "] Arithmetic: " << currentCommand << endl;
			codeWriter.writeArithmetic(parser.arg1());
		}
		else if (parser.commandType() == C_PUSH)
		{
			if (debug)
				cout << "[" << lineNumber << "] Push: " << currentCommand << endl;
			codeWriter.writePushPop("push", parser.arg1(), parser.arg2());
		}
		else if (parser.commandType() == C_POP)
		{
			if (debug)
				cout << "[" << lineNumber << "] Pop: " << currentCommand << endl;
	...
	cout << vmfile.filename() << " -> done!" << endl
		 << endl;
}
